...
import com.brickbroker.filter.JwtRequestFilter;
...
//                                // Optional: allow Swagger UI access for dev
...
//                .csrf(AbstractHttpConfigurer::disable)
//                .authorizeHttpRequests(auth -> auth
//                        .requestMatchers("/login","/register-user","/send-reset-otp","/reset-password","/logout")
//                        .permitAll()
//                        .anyRequest().authenticated())
//                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
//                .logout(AbstractHttpConfigurer::disable)
...
public class SecurityConfig {
    ...
    private CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(List.of("http://localhost:5173"));
        config.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"));
        config.setAllowedHeaders(List.of("*")); // ✅ More flexible for frontend headers
        config.setAllowCredentials(true);
